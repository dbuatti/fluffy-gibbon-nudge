...
import { getPublicAudioUrl as getPublicAudioUrlHelper, getPublicArtworkUrl as getPublicArtworkUrlHelper } from '@/integrations/supabase/client'; // Renamed getPublicArtworkUrl to avoid conflict
...
const fetchImprovisationDetails = async (supabase: SupabaseClient, id: string, sessionUserId: string): Promise<Improvisation> => {
...
const getPublicAudioUrl = (supabase: SupabaseClient, storagePath: string | null): string | null => {
    if (!storagePath) return null;
    return getPublicAudioUrlHelper(supabase, storagePath);
};

const getPublicArtworkDisplayUrl = (supabase: SupabaseClient, artworkUrl: string | null): string | null => {
    // If artwork_url is already a full URL (e.g., from manual upload or external source), use it directly.
    // Otherwise, if it's a storage path, generate the public URL.
    if (!artworkUrl) return null;
    if (artworkUrl.startsWith('http')) {
        return artworkUrl;
    }
    // Assuming artwork_url might sometimes store a path if it's from our storage bucket
    // This logic might need refinement based on how artwork is actually stored/referenced.
    // For now, if it's not an http URL, treat it as a storage path.
    return getPublicArtworkUrlHelper(supabase, artworkUrl);
};
...
const ImprovisationDetails: React.FC = () => {
  ...
  const audioPublicUrl = imp?.storage_path ? getPublicAudioUrl(supabase, imp.storage_path) : null;
  // Get public URL for the artwork
  const artworkDisplayUrl = imp?.artwork_url ? getPublicArtworkDisplayUrl(supabase, imp.artwork_url) : null;
  ...
  const handleRegenerateArtwork = async () => {
    ...
    showSuccess("AI artwork prompt generation started..."); // Updated message
    ...
    try {
      ...
      await new Promise(resolve => setTimeout(resolve, 3000)); // Give some time for prompt generation
      
      handleRefetch();
      showSuccess("New AI artwork prompt generated successfully! Check the Assets tab."); // Updated message
  ...
  if (imp) {
    ...
    if (hasAudioFile && isCoreMetadataComplete && hasNotes) {
        progressValue = 70;
        progressMessage = "Notes added. Generate artwork prompt and populate distribution fields."; // Updated message
        
        // Action 3: Generate Artwork Prompt
        if (!imp.artwork_prompt) { // Check for artwork_prompt now
            primaryAction = {
                label: "Generate AI Artwork Prompt (10% Progress Boost)", // Updated label
